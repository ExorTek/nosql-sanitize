/**
 * Express — Advanced Usage
 *
 * Demonstrates: contentTypes guard, skipRoutes with regex,
 * maxDepth, onSanitize callback, manual mode, paramSanitizeHandler
 */
...
// lobal middleware with advanced options
app.use(
  mongoSanitize({
    replaceWith: '',
    maxDepth: 5,
...
app.get('/health', (req, res) => {
  // This route is skipped — no sanitization
  res.json({ status: 'ok' });
});

app.get('/docs/swagger', (req, res) => {
  // Regex skip: /^\/docs\/.*/ — no sanitization
...
manualRouter.post('/process', (req, res) => {
  // Raw body available for inspection
...
strictRouter.post('/register', (req, res) => {
  // Only username, email, password keys pass through
  // Empty values removed, strings trimmed and lowercased

...
const Fastify = require('fastify4');
...
async function build() {
  ...
  const autoApp = Fastify({ logger: false });
  await autoApp.register(mongoSanitize);

  autoApp.post('/basic', async (request) => {
    return { body: request.body, query: request.query };
  });

  autoApp.get('/query', async (request) => {
...
build().then((app) => {
  app.listen({ port: PORT }, (err) => {
    if (err) throw err;
    console.log(`[Fastify 4] listening on :${PORT}`);
  });
});
